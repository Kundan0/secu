        ...
        for idx,frame in enumerate(track_result):# 
            ...
            if not found:
                ...
                if(returned_match is not None):
                    ...
                    l=round(model_left.predict(np.array([len(myTracks)+2]).reshape(-1,1)).item())
                    t=round(model_top.predict(np.array([len(myTracks)+2]).reshape(-1,1)).item())
                    r=round(model_right.predict(np.array([len(myTracks)+2]).reshape(-1,1)).item())
                    b=round(model_bottom.predict(np.array([len(myTracks)+2]).reshape(-1,1)).item())
                    track_=(l,t,r,b)
